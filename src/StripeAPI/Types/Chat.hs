...
import {-# SOURCE #-} StripeAPI.Types.ChatLocation
import {-# SOURCE #-} StripeAPI.Types.ChatPermissions
import {-# SOURCE #-} StripeAPI.Types.ChatPhoto
import {-# SOURCE #-} StripeAPI.Types.Message
...
data Chat = Chat {
  ...
  -- | has_private_forwards: *Optional*. True, if privacy settings of the other party in the private chat allows to use \`tg:\/\/user?id=\<user_id>\` links only in chats with the user. Returned only in [getChat](https:\/\/core.telegram.org\/bots\/api\/\#getchat).
  , chatHasPrivateForwards :: (GHC.Maybe.Maybe GHC.Types.Bool)
  -- | has_protected_content: *Optional*. True, if messages from the chat can\'t be forwarded to other chats. Returned only in [getChat](https:\/\/core.telegram.org\/bots\/api\/\#getchat).
  , chatHasProtectedContent :: (GHC.Maybe.Maybe GHC.Types.Bool)
  ...
  -- | slow_mode_delay: *Optional*. For supergroups, the minimum allowed delay between consecutive messages sent by each unpriviledged user; in seconds. Returned only in [getChat](https:\/\/core.telegram.org\/bots\/api\/\#getchat).
...
mkChat chatId chatType = Chat{chatBio = GHC.Maybe.Nothing,
                              ...
                              chatHasPrivateForwards = GHC.Maybe.Nothing,
                              chatHasProtectedContent = GHC.Maybe.Nothing,
                              chatId = chatId,
...
data ChatType' =
   ChatType'Other Data.Aeson.Types.Internal.Value -- ^ This case is used if the value encountered during decoding does not match any of the provided cases in the specification.
  | ChatType'Typed Data.Text.Internal.Text -- ^ This constructor can be used to send values to the server which are not present in the specification yet.
  | ChatType'EnumPrivate -- ^ Represents the JSON value @"private"@
  | ChatType'EnumGroup -- ^ Represents the JSON value @"group"@
  | ChatType'EnumSupergroup -- ^ Represents the JSON value @"supergroup"@
  | ChatType'EnumChannel -- ^ Represents the JSON value @"channel"@
  deriving (GHC.Show.Show, GHC.Classes.Eq)
instance Data.Aeson.Types.ToJSON.ToJSON ChatType'
    where toJSON (ChatType'Other val) = val
          toJSON (ChatType'Typed val) = Data.Aeson.Types.ToJSON.toJSON val
          toJSON (ChatType'EnumPrivate) = "private"
          toJSON (ChatType'EnumGroup) = "group"
          toJSON (ChatType'EnumSupergroup) = "supergroup"
          toJSON (ChatType'EnumChannel) = "channel"
instance Data.Aeson.Types.FromJSON.FromJSON ChatType'
    where parseJSON val = GHC.Base.pure (if | val GHC.Classes.== "private" -> ChatType'EnumPrivate
                                            | val GHC.Classes.== "group" -> ChatType'EnumGroup
                                            | val GHC.Classes.== "supergroup" -> ChatType'EnumSupergroup
                                            | val GHC.Classes.== "channel" -> ChatType'EnumChannel
                                            | GHC.Base.otherwise -> ChatType'Other val)
